...
const findMatch = (data, path, host, protocol) => {
  ...
  const match = _find__default['default'](data, o => {
    ...
    const parsed_path = url.parse(path);
    params = isAbsoluteURI(o.from) ? from(fullUri) : from(parsed_path.query == null ? parsed_path.pathname : parsed_path.pathname + parsed_path.search); // If there specific language rules, do they match
